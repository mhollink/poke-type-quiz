...
function MissedRoundFeedback({
  ...
  Pick<TypeSurvivalGameResultProps, "incorrectType" | "usedPokemonIds">
>) {
  const { availablePokemon } = usePokemonData();

  const hints: Pokemon[] = useMemo(() => {
    const unusedPokemon = availablePokemon.filter(
      (candidate) => !usedPokemonIds.has(candidate.id),
    );
    const withRequestedType = unusedPokemon.filter((candidate) =>
      candidate.types.includes(incorrectType),
    );
