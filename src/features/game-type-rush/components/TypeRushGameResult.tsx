...
import {createTypeRushChallengeShareText, shareGameResult, type ShareResult,} from "../../../utils";
import {GameResult} from "../../game-shared/components/GameResult";
import type {TypeRushAttemptRecord, TypeRushGameOverReason,} from "../model/typeRushGameTypes.ts";
    ...
    return (
        <Stack spacing={2}>
            ...
            {!!skippedTypes && !!usedPokemonIds && skippedTypes.size > 0 && (
                <SkippedRoundFeedback skippedTypes={skippedTypes} usedPokemonIds={usedPokemonIds}/>
            )}
...
function SkippedRoundFeedback({
    ...
    const hints: Pokemon[] = useMemo(() => {
        const unusedPokemon = shuffle(availablePokemon.filter(candidate => !usedPokemonIds.has(candidate.id)))

        return [...skippedTypes.values()]
            .map(typeKey => unusedPokemon.find(candidate => createTypeKey(candidate.types) === typeKey))
            .filter(value => !!value)
