...
(() => {
	...
	function createAlternateFormEntry(apiPokemon, formInfo, original) {
        ...
        const name= formInfo?.names?.find(name => name.language.name === "en")?.name ??  toDisplayName(apiPokemon.name);
	...
	async function addAlternateForms() {
		...
		for (const form of alternateForms) {
			...
			const original = pokemonData.find(({ id }) => id === apiPokemon.species.name);
