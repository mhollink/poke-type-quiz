    ...
    for (const argument of argv) {
        ...
        switch (key) {
            ...
            case '--retries':
                ...
                break;
            case '--output':
                options.outputFile = path.resolve(process.cwd(), requireValue(key, value));
                break;
            case '--checkpoint':
                options.checkpointFile = path.resolve(process.cwd(), requireValue(key, value));
                break;
            default:
...
function toMove(move) {
    ...
    if (!['physical', 'special', 'status'].includes(classifier)) {
        throw new Error(`Move ${move.name} has an unsupported damage class: ${classifier}`);
    }

    const englishName = move.names?.find(candidate => candidate.language?.name === 'en')?.name;
...
function indexMoves(moves) {
    ...
    for (const move of moves) {
        if (!Number.isInteger(move.nr)) {
            throw new Error(`Output file contains a move with an invalid nr: ${JSON.stringify(move)}`);
        }
...
async function persistProgress({ movesByNumber, checkpoint, options }) {
    const sortedMoves = [...movesByNumber.values()].sort((left, right) => left.nr - right.nr);
...
async function run(options) {
    ...
    while (checkpoint.pageUrl) {
        ...
        if (!Array.isArray(page.results)) {
            throw new Error(`Page ${checkpoint.currentPage} did not contain a results array`);
        }
        ...
        for (let index = checkpoint.nextMoveIndex; index < page.results.length; index += 1) {
            const moveReference = page.results[index];
            const progress = `${index + 1}/${page.results.length}`;

            console.log(`[page ${checkpoint.currentPage}, move ${progress}] Fetching ${moveReference.name}`,);
            ...
            if (moveResponse.meta?.category?.name !== "damage") {
                console.log(`[page ${checkpoint.currentPage}, move ${progress}] Skipping ${moveReference.name}: not a damage move`,);
...
async function main() {
    try {
        ...
        if (typeof fetch !== 'function') {
            throw new Error('This script requires Node.js 18 or newer because it uses the built-in fetch API.');
        }
